...
class SchedulerTests(unittest.TestCase):
    ...
    def test_at(self):
        mock_job = make_mock_task()
        with mock_datetime(2017, 9, 4, 12, 00):
            self.assertEqual(default_cron.new_task().every_day().at("11:30").do(mock_job).next_run_time, datetime.time(11, 30))
            self.assertEqual(default_cron.new_task().at("11:30").do(mock_job).next_run_time, datetime.time(11, 30))
    ...
    def test_run_on_days_at_time(self):
        mock_job = make_mock_task()
        with mock_datetime(2017, 9, 4, 12, 00):
            self.assertEqual(default_cron.new_task().every_day().at("12:30").do(mock_job).next_run_time, datetime.time(12, 30))
            self.assertEqual(default_cron.new_task().every_day().at("11:30").do(mock_job).next_run_time, datetime.time(11, 30))
            self.assertEqual(default_cron.new_task().every_day().at("11:30").do(mock_job).next_run_date, datetime.date(2017, 9, 5))
            self.assertEqual(default_cron.new_task().on_days("1-4").at("12:30").do(mock_job).next_run_date, datetime.date(2017, 9, 5))
            self.assertEqual(default_cron.new_task().on_days("5-6").at("12:30").do(mock_job).next_run_date, datetime.date(2017, 9, 9))

    def test_run_at_specified_time_starting_today(self):
        mock_job = make_mock_task()
        with mock_datetime(2017, 9, 4, 12, 0):
            default_cron.new_task().at("13:00").do(mock_job)
    ...
    def test_run_on_specified_days_past_today(self):
        ...
        with mock_datetime(2017, 9, 4, 12, 16):
            default_cron.new_task().on_days("0,2,4").at('12:15').do(mock_job)
            default_cron.run_pending()
    ...
    def test_run_at_specified_hours(self):
        ...
        with mock_datetime(2017, 9, 4, 9, 0):
            default_cron.new_task().at_hours("12-16").do(mock_job)
            default_cron.run_pending()
            assert mock_job.call_count == 0
        ...
        with mock_datetime(2017, 9, 4, 17, 1):
            default_cron.run_pending()
            assert mock_job.call_count == 4
        mock_job2 = make_mock_task()
        with mock_datetime(2017, 9, 4, 17, 1):
            default_cron.new_task().at_hours("12-16").do(mock_job2)
            default_cron.run_pending()
            assert mock_job2.call_count == 0

    def test_run_at_specified_hours_and_minutes(self):
        mock_job = make_mock_task()
        with mock_datetime(2017, 9, 4, 9, 0):
            default_cron.new_task().at_hours("12-14").at_minutes("15, 45").do(mock_job)
            default_cron.run_pending()
            assert mock_job.call_count == 0
    ...
    def test_run_on_specified_days_at_specified_times(self):
        mock_job = make_mock_task()
        with mock_datetime(2017, 9, 4, 9, 0):
            default_cron.new_task().on_days("2, 4").at_hours("12").at_minutes("15, 45").do(mock_job)
            default_cron.run_pending()
            assert mock_job.call_count == 0
